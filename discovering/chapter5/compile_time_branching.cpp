...
auto main() -> int {
    ...
    // if max_iter is more than 100 we will use double, otherwise we will use
    // float
    using tmp_name =
    ...
    using tmp_name_2 =
        typename conditional<(max_iter > 1000), double, float>::type;
    std::cout << "typeid = " << typeid(tmp_name_2).name() << '\n';

    // using type alias
    conditional_t<false, int, char> int_or_char = 0;
